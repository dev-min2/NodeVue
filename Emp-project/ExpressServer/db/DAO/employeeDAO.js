...
let employeeDAO = {
    ...
    selectQuery : function(empNo) {
        let selectQuery = 'SELECT A.EMP_NO, A.FIRST_NAME, A.LAST_NAME, A.GENDER, A.HIRE_DATE, D.SALARY, C.DEPT_NO, C.DEPT_NAME FROM EMPLOYEES AS A JOIN DEPT_EMP AS B ON A.EMP_NO = B.EMP_NO JOIN DEPARTMENTS AS C ON B.DEPT_NO = C.DEPT_NO JOIN SALARIES AS D ON A.EMP_NO = D.EMP_NO WHERE B.TO_DATE = CAST("9999-01-01" AS DATE) AND D.TO_DATE = CAST("9999-01-01" AS DATE) AND A.EMP_NO =' + empNo;
